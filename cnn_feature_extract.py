...
torch.manual_seed(123) #For reroducibility
# device = (
#     "cuda"
...
#Not much need for cuda here.
model = Net().apply(init_weights)

path = "haircut_data" #3006 images, 9 labels
...
feature_extractor = torch.nn.Sequential(*list(model.children())[:-2]) #running this on the data
#is the same as running all but the last of the CNN layers
print(list(model.children())[:-2]) #showing the model extracted. This is why functions are declared
#in the order they operate in the model as above.
